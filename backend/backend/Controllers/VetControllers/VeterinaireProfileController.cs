...
namespace backend.Controllers.VetControllers
{
    ...
    public class VeterinaireProfileController : ControllerBase
    {
        ...
        private readonly ILogger<VeterinaireProfileController> _logger; 

        public VeterinaireProfileController(IVeterinaireProfileRepo profileService,
            ILogger<VeterinaireProfileController> logger,
        ...
        public async Task<IActionResult> ChangePassword([FromBody] VetChangePasswordDto model)
        {
            ...
            if (veterinaire == null)
            {
                _logger.LogError($"Veterinaire profile not found for AppUserId: {user.Id} during change password.");
                return StatusCode(StatusCodes.Status500InternalServerError, new ApiResponse { Status = "Error", Message = "Veterinaire profile not found. Please contact support." });
            }
